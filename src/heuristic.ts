...
export type HeuristicScore = {
  ...
  structuralQuality: number  // 1–5
  tokens: number             // estimated token count
  format: "toon" | "hybrid" | "markdown" | "other"
  warnings: string[]         // human-readable structural issues
}
...
function detectFormat(body: string): HeuristicScore["format"] {
  ...
  if (hasToonFence && hasMarkdownContent) return "hybrid"
  if (hasToonFence) return "toon"
  if (hasMarkdownContent) return "markdown"
  ...
  if (hasToonKeyValue) return "toon"
...
function maxNestingDepth(body: string): number {
  let max = 0
...
function scoreTokenEfficiency(
  ...
  let base: number
  if (format === "toon") base = 5
  else if (format === "hybrid") base = 3
  else if (format === "markdown") base = 2
  else base = 3

  // Token count adjustments
  if (tokens > 2000) base = Math.min(base, 1)
  else if (tokens > 1200) base = Math.min(base, 2)
  else if (tokens > 700) base = Math.min(base, 3)
  else if (tokens <= 500 && format === "toon") base = Math.max(base, 5)
  ...
  if (contentLines > 0) {
    ...
    if (bulletRatio > 0.4) base = Math.max(1, base - 1)
    if (headerRatio > 0.15) base = Math.max(1, base - 1)
  }

  // Reward pipe usage in non-pure-toon files
  if (format !== "toon" && pipes >= 3) base = Math.min(5, base + 1)
...
function scoreStructuralQuality(
  maxDepth: number,
  verboseKeys: number,
  format: HeuristicScore["format"],
): number {
  ...
  let score: number
  if (format === "markdown") score = 2
  else if (format === "hybrid") score = 3
  else score = 5

  // Nesting depth penalties
  if (maxDepth > 5) score = Math.max(1, score - 2)
  else if (maxDepth > 3) score = Math.max(1, score - 1)

  // Verbose key penalties
  if (verboseKeys > 5) score = Math.max(1, score - 1)
...
function buildWarnings(
  ...
  const w: string[] = []
  if (tokens > 500) w.push(`file_over_500_tokens(~${tokens})`)
  if (maxDepth > 3) w.push(`nesting_depth_${maxDepth}_exceeds_3`)
  if (verboseKeys > 0) w.push(`${verboseKeys}_verbose_key(s)_over_15_chars`)
  if (format === "markdown") w.push("pure_markdown_use_toon_for_token_savings")
  if (format === "hybrid") w.push("hybrid_consider_moving_prose_into_toon_fence")
  if (bullets > 5) w.push(`${bullets}_markdown_bullets_replace_with_pipe_lists`)
  if (headers > 3) w.push(`${headers}_markdown_headers_replace_with_nested_keys`)
  return w
}

export function score(body: string): HeuristicScore {
...
export function formatAnnotation(s: HeuristicScore): string {
  const lines = [
